...
class Decoder(nn.Module):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, context, attn_w, function):
        ...
        embedded = self.embedding(input_var) #对input对应符号的数字构建embedding
        #print(embedded.shape)64 1 256
    
        y_all = []
        attn_w_all = []
        for i in range(embedded.size(1)):#只会执行一次
            embedded_inputs = embedded[:, i, :]
            #print(embedded_inputs.shape) 64 256
            rnn_input = torch.cat([embedded_inputs, context], dim=1)#context ： 对全部encoder中间状态attention的结果
            #print(rnn_input.shape)64 384
            rnn_input = rnn_input.unsqueeze(1) 
            #print(rnn_input.shape)64 1 384
            output, hidden = self.rnn(rnn_input, hidden)
            #print(output.shape)64 1 256
            #print(hidden.shape)1 64 256
            context, attn_w = self.attention(output, encoder_outputs)
            #print(context.shape)64 1 128
            #print(attn_w.shape)64 347
            attn_w_all.append(attn_w)
            
            context = context.squeeze(1)
            #print(context.shape)64 128
            output = output.squeeze(1) 
            #print(output.shape) 64 256
